...
public interface CategoryDao extends JpaRepository<Category,Integer> {
    ...
    SELECT 
        ...
        COUNT(CASE WHEN q.is_active = true THEN 1 END) as active,
        COUNT(CASE WHEN q.is_active = false THEN 1 END) as inactive
    FROM question q
...
""", nativeQuery = true)
    ...
    SELECT
        c.id as categoryId,
        c.category as category,
        COUNT(CASE WHEN q.is_active = true THEN 1 END) as total,
        COUNT(CASE WHEN q.diff_level = 'Easy' AND q.is_active = true THEN 1 END) as easyCount,
        COUNT(CASE WHEN q.diff_level = 'Medium' AND q.is_active = true THEN 1 END) as mediumCount,
        COUNT(CASE WHEN q.diff_level = 'Hard' AND q.is_active = true THEN 1 END) as hardCount
    FROM question q
    RIGHT JOIN category c ON q.category_id = c.id
    GROUP BY c.id, c.category
    ORDER BY c.id
